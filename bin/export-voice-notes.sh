...
#   export-voice-notes.sh --delete   export, then delete from watch (leaves a ghost
#                                     library entry until you clear it on the watch)
...
# single-instance lock so the USB-trigger and a manual run can't collide on the device
...
do_sync(){
  ...
  if [ "$DELETE" -eq 1 ]; then log "Detected. Reading $VF (--delete: will remove from watch after a verified copy; note: leaves a ghost library entry until cleared on the watch)"
  else log "Detected. Reading $VF (export only; notes stay on watch)"; fi
  ...
  while IFS=$'\t' read -r num dev kb epoch; do
    [ -z "${num:-}" ] && continue
    local id="${epoch}_${kb}" base lf saved have=0
    base="$(ts_name "$epoch")"

    # Already hold a verified local copy? Then never re-download (re-downloading is
    # wasteful and the main source of churn/failures). Just proceed to delete.
    lf="$(manifest_file "$id")"
    if [ -n "$lf" ] && valid_wav "$DEST/$lf" "$kb"; then have=1; saved="$lf"; fi
    ...
    # verified local copy in hand -> safe to delete from watch
    if [ "$DELETE" -eq 1 ] && [ "$have" -eq 1 ]; then
      if delete_from_watch "$dev" "$kb"; then deleted=$((deleted+1)); log "  - deleted $dev from watch (saved as $saved)"
